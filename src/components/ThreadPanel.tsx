  ...
  return (
    <div className="w-80 border-l border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col shrink-0">
      ...
      <div className="h-16 px-4 flex items-center justify-between border-b border-slate-200 dark:border-slate-800 shrink-0">
        <h3 className="font-semibold text-slate-900 dark:text-slate-100 text-sm">Wątek</h3>
      ...
      <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[10px] text-slate-400 dark:text-slate-500">{format(parseDbDate(message.created_at), 'HH:mm')}</span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin p-3 space-y-2">
        {loading ? (
          ...
          <p className="text-center text-xs text-slate-400 dark:text-slate-500 py-4">Brak odpowiedzi — odpowiedz pierwszy.</p>
        ) : (
          replies.map(reply => (
            <div key={reply.id} className="flex gap-2">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-1.5">
                  <span className="text-xs font-semibold text-slate-700 dark:text-slate-200">{reply.user?.name}</span>
                  <span className="text-[10px] text-slate-400 dark:text-slate-500">{format(parseDbDate(reply.created_at), 'HH:mm')}</span>
                </div>
      ...
      <div className="border-t border-slate-200 dark:border-slate-800 p-3">
        {sendError && (
          <div className="flex items-center justify-between bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 text-red-700 dark:text-red-300 text-xs rounded-lg px-2.5 py-1.5 mb-2">
        ...
        <div className="flex items-end gap-2">
          <textarea
            ...
            rows={1}
            className="flex-1 px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500 max-h-[80px]"
          />
          <button
            onClick={sendReply}
